...
dir1 = "drive/MyDrive/LIVE_DATASET/"
...
def VarianceThresholdPatchSelection(im, ref=None, patch_size=32, n_patches=32, treshold = .005):
    """
    VarianceThreshold
    ...
    :param treshold: treshold of variance (default: 0.02)
    :return: patches
    """
    w, h = im.size
    ...
    while i < n_patches:
      ...
      var_r = torch.var(patch[0])
      var_g = torch.var(patch[1])
      var_b = torch.var(patch[2])
...
class IQADataset_less_memory(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.status == 'train':
            ...
            patches = VarianceThresholdPatchSelection(im, ref, self.patch_size, self.n_patches)
            
        else:
            # patches = NonOverlappingCropPatches(im, ref, self.patch_size)
            patches = VarianceThresholdPatchSelection(im, ref, self.patch_size)
...
class IQADataset(Dataset):
    ...
    def __init__(self, args, status='train', loader=default_loader):
        ...
        for idx in range(len(self.index)):
            ...
            if status == 'train':
                ...
                patches = VarianceThresholdPatchSelection(im, ref, args.patch_size)
    ...
    def __getitem__(self, idx):
        if self.status == 'train':
            ...
            patches = VarianceThresholdPatchSelection(self.ims[idx], self.refs[idx], self.patch_size, self.n_patches)
...
def get_data_loaders(args):
    ...
    return train_loader1  ,train_loader, val_loader, test_loader, scale
...
def run(args):
    ...
    Run the program
    """
    train_loader1 , train_loader, val_loader, test_loader, scale = get_data_loaders(args)
    ...
    def log_training_results(engine):
        ...
        writer.add_scalar("SROCC/tr", SROCC, engine.state.epoch)
        writer.add_scalar("KROCC/tr", KROCC, engine.state.epoch)
        writer.add_scalar("PLCC/tr", PLCC, engine.state.epoch)
        writer.add_scalar("RMSE/tr", scale * RMSE, engine.state.epoch)
        writer.add_scalar("MAE/tr", scale * MAE, engine.state.epoch)
        writer.add_scalar("OR/tr", OR, engine.state.epoch)
...
if __name__ == "__main__":
    ...
    if args.database == 'TID2013':
        args.data_info = dir2+'TID2013fullinfo.mat'                                   
        args.im_dir = dir2+'tid2013/distorted_images/'
        args.ref_dir = dir2+'tid2013/reference_images/'
    ...
    if args.database == 'LIVE':
        args.data_info = dir1+'LIVEfullinfo.mat'
        args.im_dir    = dir1+'databaserelease2/'
        args.ref_dir   = dir1+'databaserelease2/refimgs'
